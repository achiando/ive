import { generateAssessmentFromEquipmentOpenAI } from '@/lib/actions/openai-assessment';
...
function buildOpenAIPrompt(contentSource: string, safetyTestName: string, userMessage?: string) {
...
async function callOpenAIWithRetry(messages: Array<{ role: string; content: string }>, retryCount = 0): Promise<any> {
  const maxRetries = 4;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const getEquipmentContent = async (safetyTestId: string | undefined, documentTitle: string): Promise<string> => {
      ...
      const equipmentContent = await generateAssessmentFromEquipmentOpenAI(equipmentId, actualSafetyTestName);
    ...
    if (userMessage) {
      ...
      openaiMessages = buildOpenAIPrompt(contentSource, documentTitle, userMessage);
    } else {
      if (manualUrl) {
        try {
          contentSource = await fetchManualText(manualUrl);
      ...
      openaiMessages = buildOpenAIPrompt(contentSource, documentTitle);
    }
